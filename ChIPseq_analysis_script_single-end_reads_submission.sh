...
## Title: ChIPseq_analysis_script_single-end_reads.sh    ##
###########################################################
...
# BEFORE YOU START: 
    ...
    # 5. Please update the file path in the command (Step 1.7) below before executing this script, and safe the script.
...
#################################################################################################
    ...
    cd /Users/florian/BaseSpace/071321_Sternberg_NextSeq_Chavez
...
#############################################################
    ...
    # When using single-end reads, only use -i and -o and delete -I and -O.

    for i in *1.fastq.gz; do fastp -i "$i" -o "${i/_1/_1_trimmed}" -j "${i/_1.fastq.gz/_trimmed}".json -h "${i/_1.fastq.gz/_trimmed}".html; done
...
#########################################################################################
    ...
    for i in *1_trimmed.fastq.gz; do echo ""; echo ""; echo "The file $i is being aligned..."; echo ""; echo ""; $BT2_HOME/bowtie2 -x $PWD/../reference_genome_indexed/indexed_genome -U "$i" -S $PWD/../reads_aligned_sam/"${i/_1_trimmed.fastq.gz/_aligned}".sam; done
